...
public class Mk2SwerveModuleBuilder {
     ...
     */
    private static final double DEFAULT_ANGLE_REDUCTION = 12.63 / 1.0;  // was 18.0
     ...
     * @return The builder.
     */
    public Mk2SwerveModuleBuilder angleEncoder(TalonFX motor, double angleOffset) {
        angleSupplier = () -> {
            final double sensorCoefficient = (2.0 * Math.PI) / (DEFAULT_ANGLE_REDUCTION * 2048.0);
    ...
    public Mk2SwerveModuleBuilder angleMotor(TalonFX motor, double angleOffset) {
        return angleMotor(motor, DEFAULT_FALCON_ANGLE_CONSTANTS, DEFAULT_ANGLE_REDUCTION, angleOffset);
    }

    public Mk2SwerveModuleBuilder angleMotor(TalonFX motor, PidConstants constants, double reduction, double angleOffset) {
        ...
        initializeAngleCallback = angle -> motor.getSensorCollection().setIntegratedSensorPosition(angle / sensorCoefficient, 50); // used to be wait for 50 ms
    ...
    public Mk2SwerveModuleBuilder driveMotor(SpeedController motor, MotorType motorType) {
        ...
        //motor.setInverted(true); // ROSS I think falcons are also backwards
